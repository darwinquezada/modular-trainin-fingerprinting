...
def load_test_data(valid_file_name):
    ...
    testing_x = test_data_frame.iloc[:, :-5].values
    testing_y = test_data_frame.iloc[:, -5:].values
    return testing_x, testing_y

#input data separated before
def load_data_perspective(file_name):
    data_frame = pd.read_csv(file_name)
    data_x = data_frame.iloc[:, :-5].values
    data_y = data_frame.iloc[:, -5:].values
...
def load_data_all(train,test):
    return load_data_perspective(train),load_data_perspective(test)
...
def load(train_file_name, valid_file_name):
    ...
    training_x = train_data.iloc[:, :-5].values
    training_y = train_data.iloc[:, -5:].values
    validation_x = valid_data_trame.iloc[:, :-5].values
    validation_y = valid_data_trame.iloc[:, -5:].values
    testing_x = test_data_frame.iloc[:, :-5].values
    testing_y = test_data_frame.iloc[:, -5:].values
...
def normalizeX(arr,b=2.71828):
    ...
    return normalizeX_powed(arr,b).reshape([-1, np.shape(arr)[1]])
...
class NormY(object):
    ...
    def reverse_normalizeY(self,longitude_arr, latitude_arr, altitude_arr):
        ...
        if self.alti_scale == 0:
            ...
            alti = (altitude_arr * self.alti_scale) + self.alti_min
