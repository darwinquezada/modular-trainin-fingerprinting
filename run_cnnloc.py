...
from test.test_cnnloc import test_cnnloc
...
def run_cnnloc(dataset_name=None, ap_selection=None, path_config=None, dataset_config=None, cnnloc_config=None):
    ...
    if np.shape(temp2)[1] > 1:
        unique_ap_list = pd.DataFrame(
            temp2.iloc[:, 0].drop_duplicates().reset_index(drop=True))
    else:
    ...
    # Test
    path_results = os.path.join(path_config['results'], dataset_name, 'CNNLoc', str(ap_selection['num_aps']))
    test_cnnloc(y_train=train_y, X_test=test_x, y_test=test_y, path_saved_models=path_data_model,
