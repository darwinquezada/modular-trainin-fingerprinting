...
from test.test_cnn_lstm import test_cnn_lstm
...
def run_cnn_lstm(dataset_name=None, ap_selection=None, path_config=None, dataset_config=None, cnn_lstm_config=None):
    '''
    ...
    path_data_model = os.path.join(path_config['saved_model'], dataset_name, 'CNN-LSTM', str(ap_selection['num_aps']))
    ...
    for i in range(0, np.shape(unique_ap_list)[0]):
        ...
        if not os.path.exists(path_save_info_ap):
            ...
            lab_encoded_floor = encoding_floor.fit_transform(selected_y_train['FLOOR'])
            selected_y_train['FLOOR'] = lab_encoded_floor.reshape(-1, 1)
            ...
            lab_encoded_building = encoding_building.fit_transform(selected_y_train['BUILDINGID'])
            selected_y_train['BUILDINGID'] = lab_encoded_building.reshape(-1, 1)
            joblib.dump(encoding_building, os.path.join(path_save_info_ap, 'building_label_encoder.save'))
            ...
            joblib.dump(building_ohe_model, os.path.join(path_save_info_ap, 'building_ohe_encoder.save'))
            ...
            cnn_lstm_model = CNN_LSTM(X_data=X_data, y_data=y_data, 
                                    building_config=cnn_lstm_config[0], 
                                    floor_config=cnn_lstm_config[1],
                                    position_config=cnn_lstm_config[2])
    ...
    path_results = os.path.join(path_config['results'], dataset_name, 'CNN-LSTM', str(ap_selection['num_aps']))
    test_cnn_lstm(X_test, y_test, path_data_model, path_results, ap_selection, cnn_lstm_config)
    print("END")
    return True
